  ...
  btn.addEventListener("click", () => {
    ...
    accordionButtons.forEach((other) => {
      if (other !== btn) {
        ...
        p.classList.remove("bg-accent");
        p.classList.add("bg-transparent");
        p.querySelector(".accourdion-content").classList.add("hidden");
        other.classList.remove("!pb-0");
        other.querySelector("svg").innerHTML = plusIcon;
      }
    });

    // Toggle current
    parent.classList.toggle("bg-accent");
    parent.classList.toggle("bg-transparent");
    content.classList.toggle("hidden");
    btn.classList.toggle("!pb-0");
    svg.innerHTML = isActive ? plusIcon : minusIcon;
  });
});
